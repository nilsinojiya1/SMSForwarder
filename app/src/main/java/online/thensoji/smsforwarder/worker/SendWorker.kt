...
class SendWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        // Idempotency: Prevent sending the same message more than once
        if (messageObj.isSent) {
            Log.d(TAG, "Message $messageId is already sent. Skipping duplicate send.")
        ...
        if (botToken.isNullOrEmpty() || chatId.isNullOrEmpty()) {
            Log.e(TAG, "Bot token or chat ID is not set.")
        ...
        return try {
            when (val result = sendTelegramMessageUseCase(botToken, chatId, payload)) {
                is SendResult.Success -> {
                    ...
                    Log.d(TAG, "Message $messageId forwarded successfully (Delay: ${delayMillis}ms)")
                    Result.success()
                }
                is SendResult.Error -> {
                    Log.e(TAG, "Failed to forward message $messageId: ${result.errorMessage}", result.throwable)
                    dao.update(messageObj.copy(errorMessage = result.errorMessage))
                    Result.retry()
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error forwarding message $messageId", e)
            dao.update(messageObj.copy(errorMessage = e.localizedMessage))
            Result.retry()
        }
    }
}
